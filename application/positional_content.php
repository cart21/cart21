...
class positional_content extends CI_Controller {
	...
	function upload($positional_content_id){
	
		
	
		if($_FILES["ImageFile"]["error"][0]!=4){
		
		$directoryPath = $_SERVER["DOCUMENT_ROOT"]."/uploads/positional_content/".$positional_content_id;
		
        if (!file_exists($directoryPath)) {
        mkdir($directoryPath, 0755);
        }
		
        $config['upload_path'] ='./uploads/positional_content/'.$positional_content_id.'/';
		...
		foreach($result["success"] as $image){
			...
			$image_data=array(
			"image_loc"=>'uploads/positional_content/'.$positional_content_id.'/'.$image["file_name"],
			"positional_content_id"=>$positional_content_id,
			"title"=>str_replace($image["file_ext"],"",$image["client_name"]),
			"file_ext"=>substr($image["file_ext"],1),
			"date_added"=>mktime()
			);
			$this->db->insert("positional_content_image",	$image_data	);
