...
class email_template {
	...
	function set_Template($data){
		...
		$Template = $this->CI->db->query("SELECT *  FROM email_template WHERE email_template_id=".$data["template_id"]);
	...
	function order($order_id){
		...
		//$this->data["order"]=$this->CI->order_model->order($order_id)->row_array();
		...
		$this->data["payment_type"]=$this->CI->order_model->order_status($this->data["order"]["order_status"])->row()->title;
		$this->data["shipping_status"]=$this->CI->order_model->order_shipping_status($this->data["order"]["shipping_status"])->row()->title;
		
		if($this->data["order"]["order_status"]=="1"){
		$this->data["bank"]=$this->CI->order_model->bank($this->data["order"]["bank_id"])->row()->bank_name;
		}else{
			$this->data["bank"]="";
		}
		
		$this->data["shipping_company"]=$this->CI->order_model->shipping_company($this->data["order"]["shipping_company_id"])->row()->company_name;
